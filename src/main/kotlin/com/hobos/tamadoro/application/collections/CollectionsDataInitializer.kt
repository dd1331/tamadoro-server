...
class CollectionsDataInitializer(
    ...
    private fun seedBackgrounds() {
        if (backgroundRepository.count() > 0) return
        backgroundRepository.saveAll(
            listOf(
        ...
        log.info("Seeded backgrounds")
    }

    private fun seedMusic() {
        if (musicTrackRepository.count() > 0) return
        musicTrackRepository.saveAll(
            listOf(
                MusicTrackEntity(
                    id = "m1",
                    title = "Rain",
    ...
    private fun seedCharacters() {
        ...
        val drogo = TamagotchiCatalogEntity(
            id = "char_002",
            title = "Drogo",
            theme = "dragon",
            isPremium = false,
        ...
        characterRepository.saveAll(listOf(pippo, drogo))
        ...
        characterStageRepository.saveAll(stages)
        log.info("Seeded characters and stages")
    }
}
